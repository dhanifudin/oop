...
% (monochrome; apt package `fonts-symbola` on the GitHub Actions
% `ubuntu-latest` CI runner that builds and publishes these PDFs to Pages)
% -> DejaVu Sans as a last resort (covers warning-sign ⚠ but not the
% checkmark ✅, which would tofu there). Deliberately NOT using a color
% emoji font (tried Noto Color Emoji here first): its bitmap/color glyph
% tables aren't plain TrueType outlines, and lualatex's font loader
% crashed trying to read it ("loca table not found") rather than falling
% through gracefully, so \IfFontExistsTF alone can't protect against it --
% only genuinely monochrome, outline-glyph fonts belong in this chain.
\usepackage{fontspec}
